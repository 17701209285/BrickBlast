using UnityEngine;

public enum AimBoundaryType
{
    None = 0,
    Left = 1,
    Right = 2,
    Top = 3,
    Bottom = 4
}

public struct AimPreviewSegment
{
    public Vector2 StartPoint;
    public Vector2 EndPoint;
    public bool HitBoundary;
    public Vector2 BoundaryHitPoint;
    public AimBoundaryType BoundaryType;

    public AimPreviewSegment(
        Vector2 startPoint,
        Vector2 endPoint,
        bool hitBoundary,
        Vector2 boundaryHitPoint,
        AimBoundaryType boundaryType)
    {
        StartPoint = startPoint;
        EndPoint = endPoint;
        HitBoundary = hitBoundary;
        BoundaryHitPoint = boundaryHitPoint;
        BoundaryType = boundaryType;
    }
}

public struct AimPreviewPath
{
    public AimPreviewSegment PrimarySegment;
    public AimPreviewSegment ReflectionSegment;
    public bool HasReflectionSegment;
}

public struct AimPreviewImpactData
{
    public bool HasBlockImpact;
    public bool IsReflectionImpact;
    public Vector2 BlockImpactPoint;
    public Vector2 BlockImpactNormal;
    public ChessElement HighlightBlock;
    public ChessElement AdditionalHighlightBlock;

    public AimPreviewImpactData(
        bool hasBlockImpact,
        bool isReflectionImpact,
        Vector2 blockImpactPoint,
        Vector2 blockImpactNormal,
        ChessElement highlightBlock,
        ChessElement additionalHighlightBlock = null)
    {
        HasBlockImpact = hasBlockImpact;
        IsReflectionImpact = isReflectionImpact;
        BlockImpactPoint = blockImpactPoint;
        BlockImpactNormal = blockImpactNormal;
        HighlightBlock = highlightBlock;
        AdditionalHighlightBlock = additionalHighlightBlock;
    }

    public bool Highlights(ChessElement chessElement)
    {
        return chessElement != null && (HighlightBlock == chessElement || AdditionalHighlightBlock == chessElement);
    }
}
